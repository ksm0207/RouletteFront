<template>
  <div>
        ...
        <el-row>
          <el-col :span="6" v-if="secretYn">
            <el-form-item label="Password">
              <el-input type="password" size="small" :style="{ width: '200px', margin: '0 10px' }" v-model="passwd"></el-input>
            </el-form-item>
          </el-col>
    ...
    <el-card v-for="(item, index) in this.displayedEntries" :key="index">
      <p class="isSecretText" v-if="item.secretYn === 'true'" @click="isSecretItem(item)">비밀 글 입니다</p>
      <p v-else>
        {{ item.message }}
      </p>
    ...
    <el-pagination layout="prev, pager, next" :total="guestbookEntries.length" :page-size="pageSize" @current-change="handlePageChange"></el-pagination>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      await axios
        .post('/guestBook/submit', newGuestBookData)
        .then(response => {
    ...
    async currentViewOnCheck() {
      await axios
        .post('/guestBook/getGuestBookAllList')
        .then(response => {
    ...
    isSecretItem(item) {
      ...
      Swal.fire({
        ...
        preConfirm: async data => {
          ...
          try {
            await axios
              .post('/guestBook/secretGuestBook', getSecretGuestBook)
              .then(response => {
                if (response.data !== '') {
